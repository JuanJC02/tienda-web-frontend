...
export default function Galeria() {
  ...
  const [uploadReady, setUploadReady] = useState(null); // base64 listo para subir
      ...
      if (left <= 0) {
        ...
        if (fotos.length > 0) {
          ...
          if (winner.likes.length > 0) {
            ...
            addCodigoToUser(winner.userId, codigo);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setUploadReady(ev.target.result);
    };
  ...
  const handleUpload = () => {
    ...
    setUploading(true);
    setTimeout(() => {
      subirFoto(user.id, user.nombre, uploadReady);
  ...
  const userAlreadyUploaded = fotos.some((f) => f.userId === user?.id);
      ...
      <div className="galeria-inner">
        ...
        {ganador && (
          <div className={`ganador-banner ${ganador.sinLikes ? "sin-likes" : ""}`}>
            {ganador.sinLikes ? (
              <p>⚠️ La ronda terminó sin likes — nadie ganó esta vez. ¡Anímate a participar!</p>
            ) : (
              <p>
                🏆 ¡<b>{ganador.foto.userName}</b> ganó la ronda con {ganador.foto.likes.length} like{ganador.foto.likes.length !== 1 ? "s" : ""}
                y recibió el código <b>xxxx-xxxx-xxxx{/*{ganador.codigo.codigo} */}</b> ({ganador.codigo.porcentaje}% OFF)!
              </p>
            )}
            ...
            <div className="upload-card">
              ...
              {userAlreadyUploaded ? (
                <div className="already-uploaded">
                  ✅ Ya tienes una foto en esta ronda. ¡Pide a otros que te den like!
                </div>
              ) : uploadPreview ? (
                <div className="upload-preview-wrap">
                  ...
                  <div className="upload-preview-actions">
                    ...
                    <button className="btn-outline" onClick={cancelPreview} style={{ padding: "10px 16px" }}>
                      Cancelar
                    </button>
                  </div>
                </div>
          ...
          <div className="fotos-area">
            {fotos.length === 0 ? (
              <div className="no-fotos">
                ...
                <p>¡Sé el primero en subir una foto y comienza la competencia!</p>
              </div>
            ) : (
              <div className="fotos-grid">
                {sortedFotos.map((foto) => {
                  const liked = foto.likes.includes(user?.id);
                  const isOwn = foto.userId === user?.id;
                  const isLeader = foto.id === sortedFotos[0]?.id && foto.likes.length > 0;
                  return (
                    <div className={`foto-card ${isLeader ? "leader" : ""}`} key={foto.id}>
                      ...
                      <div className="foto-footer">
                        ...
                        <button
                          ...
                          onClick={() => !isOwn && likePhoto(foto.id, user?.id)}
                          disabled={isOwn}
